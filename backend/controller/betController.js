...
//   try {
//     const { userId, gameName, bids, totalBidPoints } = req.body;
    
//     if (!userId || !gameName || !bids || bids.length < 1 || totalBidPoints <= 0) {
//       return res.status(400).json({
//         success: false,
//         message: 'Invalid input: A game name, at least 3 bids, and a positive total bid amount are required.',
//       });
//     }
...
//     // Validate input data
//     if (!userId || !gameName || !bidType || !session || !bids || bids.length < 1 || totalBidPoints <= 0 || estimatedProfit < 0) {
//       return res.status(400).json({
//         success: false,
...
exports.placeBid = async (req, res) => {
  try {
    ...
    const sessionValue = session || 'default_session';  // Use 'default_session' if no session is provided
